...
```
Fix different values for the blood pressure level variable to have them equal
```{r}
...
diabetes_dataset__2019$Smoking_num <- ifelse(ds$Smoking == 'yes', 1, 0)
...
for(i in 1:ncol(df_cor)) {       # for-loop over columns
  print(cat(names(df_cor)[i], unique(df_cor[,i])))
}
```
