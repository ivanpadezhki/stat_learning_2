...
```
From the correlation plot, we can see that certain variables are highly correlated. Let's check which ones have a correlation above .90 (arbitrarily chosen so we can see multicollinearity) AND we select only the upper triangle
```{r}
...
Potentially concentrate on a subset of the data: the set contains mean and standard dev of 10 main characteristics
