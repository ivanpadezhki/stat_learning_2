...
ds_norm <- ds_norm %>% rename(Pregnancies=Pregancies)

#Fixing the variable Pdiabetes 
ds_norm$Pdiabetes[ds_norm$Pdiabetes == ''] <- NA
ds_norm$Pdiabetes[ds_norm$Pdiabetes == ' no'] <- 'no'
ds_norm$Pdiabetes[ds_norm$Pdiabetes == '0'] <- 'no'

#Fixing the variable Diabetic
ds_norm$Diabetic[ds_norm$Diabetic == ' no'] <- 'no'
ds_norm$Diabetic[ds_norm$Diabetic == ''] <- NA

#Fixing the variable BPLevel
ds_norm$BPLevel[ds_norm$BPLevel == 'High'] <- 'high'
ds_norm$BPLevel[ds_norm$BPLevel == 'Low'] <- 'low'
ds_norm$BPLevel[ds_norm$BPLevel == 'normal '] <- 'normal'

# Fixing the variable RegularMedicine
ds_norm$RegularMedicine[ds_norm$RegularMedicine == 'o'] <- 'no'
```

## 2.2. Identifying Missing Values

The whole data set contains 47 missing values and in light of the relatively large number of observations, the rows containing NA values will be removed from the data set prior to analysis. 
```{r}
sum(is.na(ds_norm)==TRUE)
```
...
```{r}
table(ds$Diabetic) - table(ds_norm$Diabetic)
```
## 2.3. Identifying weird observations
...
```{r}
preg_male <- subset(ds_norm, Gender == 'Male' & Pregnancies != 0)
...
ds_pre<-ds_norm[!(ds_norm$Gender == 'Male' & ds_norm$Pregnancies != 0),]
sum(is.na(ds_pre)==TRUE)
```


## 2.4.  Turning categorical variables into single-level dummies

An important step of the preparation of the data for following analyses is the transformation of categorical variables into dummies. Binary variables with yes/no as levels are coded as 1 versus 0. For variables with more than two levels, the lowest level is taken as baseline and the rest are contrasted with it. The reason for this choice of dummy coding is that none of the categorical variables are nominally scaled, rather they are ordinal variables without measurable distances between different levels. Therefore, coding them as numerical variables is not appropriate. 
...
Finally, the data set contains 893 observations with 26 predictor variables and one outcome variable. 
```{r}
dim(ds_fin)
```
...
```{r, echo = False}
# build plots
bar_smoking <- ggplot(data = ds_pre, aes(x = Smoking, fill = Diabetic)) +
    geom_bar() + theme(legend.position = "none")
bar_alc <- ggplot(data = ds_pre, aes(x = Alcohol, fill = Diabetic)) +
    geom_bar() + theme(legend.position = "none")
bar_med <- ggplot(data = ds_pre, aes(x = RegularMedicine, fill = Diabetic)) +
  geom_bar()
# get legend
legend <- get_legend(bar_med)
# delete legend from past plot
bar_med <- bar_med + theme(legend.position="none")
grid.arrange(bar_smoking, bar_alc, bar_med, legend, ncol = 3, top=textGrob("Bar Plots of Categorical Variables"))

```

```{r}
# build plots
bar_ <- ggplot(data = ds_pre, aes(x = JunkFood, fill = Diabetic)) +
    geom_bar() + theme(legend.position = "none")
bar_act <- ggplot(data = ds_pre, aes(x = PhysicallyActive, fill = Diabetic)) +
    geom_bar() + theme(legend.position = "none")
bar_stress <- ggplot(data = ds_pre, aes(x = Stress, fill = Diabetic)) +
  geom_bar()
# get legend
legend <- get_legend(bar_stress)
# delete legend from past plot
bar_stress <- bar_stress + theme(legend.position="none")
grid.arrange(bar_junk, bar_act, bar_stress, legend, ncol = 3, top=textGrob("Bar plots of categorical Variables"))

```
```{r}
# build plots
bar_urin <- ggplot(data = ds_pre, aes(x = UriationFreq, fill = Diabetic)) +
    geom_bar() + theme(legend.position = "none")
bar_bplev <- ggplot(data = ds_pre, aes(x = BPLevel, fill = Diabetic)) +
    geom_bar() + theme(legend.position = "none")
bar_highbp <- ggplot(data = ds_pre, aes(x = highBP, fill = Diabetic)) +
  geom_bar() 
# get legend
legend <- get_legend(bar_highbp)
# delete legend from past plot
bar_highbp <- bar_highbp + theme(legend.position="none")
grid.arrange(bar_urin, bar_bplev, bar_highbp, legend, ncol = 3, top=textGrob("Bar plots of categorical Variables"))
```
...
```
This correlation plot is in fact better, we also have access to the correlation coefficients. 
```{r}
library(ggcorrplot)
...
```{r}
aov_age <- aov(ds_fin$Diabetic~ds_pre$Age)
