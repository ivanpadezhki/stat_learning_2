...
high_cor
```
There are two variables highBP and BPnormal, which carry similar information - we will later check which one to exclude from the analysis
...
best.bic <- glm(Diabetic~Family_Diabetes+RegularMedicine+Pdiabetes+Age40_49+Age50_59+Age_60+PhysLow + PhysMid + StressSometimes+ BPNormal, data=ds_fin, family = binomial)
summary(best.bic)
```
...
Look at relationship between Pdiabetes and Pregnancies - seems like the number of pregnancies increases the chance of Gestational Diabetes - important to consider their relationship in the model!
```{r}
boxplot(Pregancies~Pdiabetes, data = ds_fin, main = "Pregnancies and Gestational Diabetes",ylab = "Number of Pregnancies", xlab = "Gestational Diabetes", names = c('No', "Yes"))
```
